...
class BoundedBackend(LiteLLMBackend):
    ...
    async def acomplete(self, **kwargs):
        ...
        with (HERE/'live_final/raw_responses.jsonl').open('a') as handle:
...
def main():
    ...
    models = yaml.safe_load((HERE / 'models_final.yaml').read_text())
    out = HERE / ('live_final' if args.backend=='real' else 'offline_final')
    ...
    import shutil
    # New prompt contract: fresh cache, no previous-version response reuse.
