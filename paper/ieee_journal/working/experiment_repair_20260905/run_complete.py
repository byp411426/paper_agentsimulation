...
sys.path.insert(0, str(HERE / "implementation_final"))
os.chdir(SOURCE)
import yaml
...
class BoundedBackend(LiteLLMBackend):
    ...
    async def acomplete(self, **kwargs):
        ...
        with (HERE/'live_complete/raw_responses.jsonl').open('a') as handle:
            handle.write(json.dumps({'request':request_id,'content':response.content,
...
def main():
    ...
    models = yaml.safe_load((HERE / 'models_recovery.yaml').read_text())
    out = HERE / ('live_complete' if args.backend=='real' else 'offline_complete')
    ...
    import shutil
    # Same behavior and prompts: reuse only exact successful requests after transient provider error.
    if args.backend == 'real':
        shutil.copy2(HERE/'live_final/cache.sqlite', out/'cache.sqlite')
    cache = LLMCache(out / 'cache.sqlite')
    gateway = LLMGateway(run_id=cfg['run']['run_id'], models_cfg=models,
                         budget_usd=cfg['llm']['budget_usd'], max_concurrency=4,
    ...
    initial = {'world': components[3].snapshot(),
               ...
               'graph_edges': sorted([sorted(edge) for edge in components[4].graph.edges()]),
    ...
    def deadline(signum, frame):
        raise TimeoutError('30-minute diagnostic wall-clock limit reached')
