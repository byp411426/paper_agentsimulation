    ...
    def _accept_party_proposal(
        ...
        *,
        ...
        commitment = HouseholdCommitmentV2(
            ...
            party=party,
            accepted_by=frozenset({message["sender_id"]})
            | (frozenset(message["recipient_ids"]) if not self_accept else frozenset()),
            created_step=step,
        )
        ...
        for message in self.messages.values():
            ...
            for recipient_id in message["recipient_ids"]:
                ...
                if rng.random() >= probability:
                    continue
                recipient.deliver_message(message)
                message["delivered_step"] = clock.t
