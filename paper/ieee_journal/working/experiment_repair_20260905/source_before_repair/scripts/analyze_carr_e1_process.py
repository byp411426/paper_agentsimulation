...
summary={'status':status.get('status'),'completed_steps':len(events),'planned_steps':planned,'households':rows,'metrics':metrics[-1],'contract_violations':violations,'rejection_counts':rejection_counts,'call_status_counts':calls,'gateway':status.get('gateway',{}),'provider_requests':status.get('provider_requests'),'wall_seconds':status.get('elapsed_seconds'),'action_counts':dict(action_counts),'private_plan_users':len({a['id'] for e in events for a in e['agents'] if a.get('current_plan')}),'number_of_joint_commitment_versions':sum(len(c['party']['traveler_ids'])>=2 for h,c in allcommits.values()),'number_of_solo_commitment_versions':sum(cid.startswith('auto_') for cid in allcommits),'all_commitment_final_statuses':dict(collections.Counter(c['status'] for h,c in allcommits.values()))}
...
for ax in axs[:2]:
 ax.set(xlim=(0,planned),xlabel='时间步（每步 30 分钟）');ax.set_xticks(range(0,planned+1,2));ax.grid(axis='y',alpha=.17);ax.legend(loc='upper left',fontsize=8,frameon=False)
 for tick,label in [(3,'建议撤离'),(7,'强制命令'),(9,'主路封闭')]:
  ax.axvline(tick,color='#aaa',ls=':',lw=.9);ax.text(tick+.08,ax.get_ylim()[1]-.1,label,rotation=90,va='top',fontsize=8,color='#777')
...
ids=sorted(deciders);palette=['#edf0f3','#a9c8e7','#f1ce63','#269b7a','#d86c63','#a28fca','#68b9b8'];codes={'stay':1,'prepare':2,'seek_help':5,'offer_help':6}
...
fig,ax=plt.subplots(figsize=(13,4.2),layout='constrained');ax.imshow(arr,aspect='auto',cmap=ListedColormap(palette),norm=BoundaryNorm([v-.5 for v in range(8)],7),interpolation='nearest');ax.set_xticks(range(len(events)),[e['step'] for e in events]);ax.set_yticks(range(len(ids)),[labels[member_h[r]]+' / '+r.rsplit('_',1)[-1] for r in ids]);ax.set(title='逐人行动与实际执行结果',xlabel='时间步');ax.legend(handles=[Patch(facecolor=c,label=t) for c,t in zip(palette,['本步未唤醒','留守 / 监测','准备','已执行出发','出发被拒绝','求助','提供帮助'])],loc='upper center',bbox_to_anchor=(.5,-.17),ncol=4,frameon=False,fontsize=8);fig.savefig(P/'actions_preview.png',dpi=160,bbox_inches='tight');plt.close(fig)
