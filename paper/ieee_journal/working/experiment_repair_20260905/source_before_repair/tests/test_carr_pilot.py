...
def test_v8_formal_protocol_is_frozen_real_backend_and_test_sealed():
    ...
    # W4 (post-r7-archive) legitimately extends shared core with additive v2
    # interfaces.  The archived E2 baseline is recorded in the r7 freeze; legacy
    # E2 semantics are preserved by the household/interaction regression tests,
    # so byte-identity is required only for files not extended by W4.
    w4_extended = {
        "ds/households/state.py",
    }
    for source in freeze["frozen_sources"]:
        path = PROJECT_ROOT / source["path"]
        assert path.exists()
        if source["path"] in w4_extended:
            continue
        assert file_sha256(path) == source["sha256"]
