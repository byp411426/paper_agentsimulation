...
class BoundedBackend(LiteLLMBackend):
    ...
    async def acomplete(self, **kwargs):
        ...
        with (HERE/'live_r2/raw_responses.jsonl').open('a') as handle:
...
def main():
    ...
    out = HERE / ('live_r2' if args.backend=='real' else 'offline_r2')
    ...
    import shutil
    if args.backend=='real': shutil.copy2(HERE / 'live/cache.sqlite', out / 'cache.sqlite')
