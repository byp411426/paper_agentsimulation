...
def main():
    with (Q / 'batch_execution.lock').open('a') as lock:
        ...
        replay = json.loads((Q / 'amendments/concurrency_limit_3/offline_replay_check.json').read_text())
        ...
        if not (Q / 'runs/100hh_seed101/accepted.json').exists():
            assert not (Q / 'runs/100hh_seed101/attempt2').exists(), 'Do not restart an existing attempt'
            command = [sys.executable, str(Q / 'recover_job.py'), '--index', '3',
                       '--attempt', '2', '--resume-cache',
                       str(Q / 'runs/100hh_seed101/attempt1/cache.sqlite')]
