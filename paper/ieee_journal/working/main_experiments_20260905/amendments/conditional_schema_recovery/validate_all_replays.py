...
def main():
    ...
    target = Q / 'amendments/conditional_schema_recovery'
    checks = []
    for households, seed, attempt in [(24, 7201, 3), (24, 8301, 1), (24, 9401, 1), (100, 101, 2)]:
        ...
        sys.argv = ['worker.py', '--households', str(households), '--seed', str(seed),
                    '--attempt', '904', '--backend', 'mock', '--resume-cache', str(cache)]
        with (target / f'offline_{households}hh_seed{seed}.log').open('w') as log:
            with contextlib.redirect_stdout(log), contextlib.redirect_stderr(log):
                rc = worker.main()
        replay = Q / f'offline/{households}hh_seed{seed}/attempt904'
