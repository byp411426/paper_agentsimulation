    ...
    def try_accept_commitment_v2(
        ...
        *,
        ...
        if any(active.id!=supersedes_id for active in overlaps):
            conflict=next(active for active in overlaps if active.id!=supersedes_id)
            return CommitmentAcceptanceResult(False,None,'vehicle_in_use' if conflict.party.vehicle_id==party.vehicle_id else 'member_overlap')
