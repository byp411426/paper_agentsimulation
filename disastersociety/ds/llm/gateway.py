...
Five guarantees:
  ...
  5. Structured output: decisions are JSON validated by a pydantic schema;
     one repair by default, at most two when explicitly configured.
...
def _normalize_model_json(text: str) -> tuple[str, str | None]:
    """Remove exactly one redundant closing brace after a complete JSON object.

    No fields are changed. Additional objects, prose, truncated objects, or
    other trailing content remain invalid and go through normal validation.
    """
    ...
    if isinstance(value, dict) and cleaned[end:].strip() == "}":
        return cleaned[:end], "one_extra_closing_brace"
    return cleaned, None
...
class LLMGateway:
    ...
    async def complete(
        ...
        *,
        ...
        max_schema_repairs = int(self.models[model].get("max_schema_repairs", 1))
        if not 1 <= max_schema_repairs <= 2:
            raise ValueError("max_schema_repairs must be 1 or 2")
        t0 = time.time()
        ...
        for correction_index in range(max_schema_repairs + 1):
            try:
                out = schema.model_validate_json(normalized)
                break
            except (json.JSONDecodeError, ValidationError) as validation_error:
                if correction_index == max_schema_repairs:
                    self.n_failed += 1
                    self._record(dict(run_id=self.run_id, decision_id=decision_id,
                        step=step, agent_id=agent_id, model=model, key=key,
                ...
                if correction_index and self.spent >= self.budget:
                    self.flush()
                    raise BudgetExceeded(f"run {self.run_id} exhausted budget before additional schema repair")
                validation_details = (
                    json.dumps(validation_error.errors(include_input=False, include_url=False), default=str)
                ...
                fix_msgs = messages + [
                    ...
                    {"role": "user", "content":
                     ...
                     "Satisfy these conditional requirements as well as the schema. "
                     "Use only resources actually present in the supplied observations; "
                     "if a chosen action cannot be specified without inventing resources, select a supported feasible action. "
                     "Output ONLY the corrected JSON object, no explanation. "
                     f"Every enum value must match this JSON Schema exactly: {repair_schema}"},
                ]
                if correction_index:
                    guidance = getattr(schema, "final_repair_guidance", None)
                    fix_msgs[-1]["content"] += (
                        " This is the final allowed schema correction. "
                        "A nullable field in the JSON Schema can still be required by "
                        "the conditional validation errors above. "
                        + (guidance() if callable(guidance) else "")
                    )
                try:
                    async with self.sem:
                        for repair_attempt in range(max_attempts):
                            try:
                                ...
                                self._record(dict(run_id=self.run_id, decision_id=decision_id,
                                    step=step, agent_id=agent_id, model=model, key=key,
                                    status="provider_retry", phase="schema_repair", repair_round=correction_index + 1, attempt=repair_attempt + 1,
                                    retry_after_seconds=delay, error=str(e), **seed_record))
                                self.flush()
                                await asyncio.sleep(delay)
                except BackendFatalError as e:
