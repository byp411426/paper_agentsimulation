"""Run one detached r7 lane without cross-attempt output-name collisions.

Each seed batch is written under the r7 protocol id.  The supervisor never
retries a seed batch and stops before starting another paid batch after any
exception or non-five-for-five-VALID result.  r7 is the frozen full 60-cell
recovery after r4/r5/r6 stopped on intermittent provider failures; science
configuration is identical to r6, and the transport config adds bounded
retries with minute-scale backoff (max_attempts 5, [5,15,30,60] seconds).
"""
...
def main() -> int:
    ...
    if protocol_id != "carr_s_e2_v8_formal_r7":
        raise ValueError("r7 lane runner requires the frozen r7 protocol")
