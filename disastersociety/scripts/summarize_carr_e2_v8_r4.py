"""Audit r4 batches through the frozen v8 summarizer without renaming data."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "experiments/carr/configs/carr_s_e2_v8_formal_r4.yaml"
DEFAULT_RESULTS = PROJECT_ROOT / "experiments/carr/results"
DEFAULT_RUNS = PROJECT_ROOT / "experiments/carr/runs"
DEFAULT_OUTPUT = DEFAULT_RESULTS / "carr_s_e2_v8_formal_r4_cumulative_summary.json"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(
        prefix="carr-e2-r4-audit-", dir=results_dir
    ) as temporary:
        audit_dir = Path(temporary)
        for source in results_dir.glob(
            "carr_s_e2_v8_formal_r4_seed*_summary.json"
        ):
            seed = source.stem.removeprefix(
                "carr_s_e2_v8_formal_r4_seed"
    ...
    if summary["completion"]["planned_runs"] != (
        ...
        raise ValueError("r4 audit planned-run count violates E2 contract")
    output.write_text(
