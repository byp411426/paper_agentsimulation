"""Run one detached r5 lane without cross-attempt output-name collisions.

Each seed batch is written under the r5 protocol id.  The supervisor never
retries and stops before starting another paid batch after any exception or
non-five-for-five-VALID result.  r5 is the frozen full 60-cell recovery after
r4 stopped on two provider connection errors; science configuration is
identical to r4 except for protocol/run identity.
"""
...
def main() -> int:
    ...
    if protocol_id != "carr_s_e2_v8_formal_r5":
        raise ValueError("r5 lane runner requires the frozen r5 protocol")
