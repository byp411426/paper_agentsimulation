"""Audit r6 batches through the frozen v8 summarizer without renaming data."""
...
DEFAULT_CONFIG = PROJECT_ROOT / "experiments/carr/configs/carr_s_e2_v8_formal_r6.yaml"
DEFAULT_RESULTS = PROJECT_ROOT / "experiments/carr/results"
DEFAULT_RUNS = PROJECT_ROOT / "experiments/carr/runs"
DEFAULT_OUTPUT = DEFAULT_RESULTS / "carr_s_e2_v8_formal_r6_cumulative_summary.json"
...
def main() -> None:
    ...
    protocol_id = "carr_s_e2_v8_formal_r6"
    with tempfile.TemporaryDirectory(
        prefix="carr-e2-r6-audit-", dir=results_dir
    ) as temporary:
        audit_dir = Path(temporary)
        for source in results_dir.glob(f"{protocol_id}_seed*_summary.json"):
    ...
    if summary["completion"]["planned_runs"] != (
        ...
        raise ValueError("r6 audit planned-run count violates E2 contract")
    output.write_text(
