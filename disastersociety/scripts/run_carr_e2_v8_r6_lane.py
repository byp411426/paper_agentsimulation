"""Run one detached r6 lane without cross-attempt output-name collisions.

Each seed batch is written under the r6 protocol id.  The supervisor never
retries a seed batch and stops before starting another paid batch after any
exception or non-five-for-five-VALID result.  r6 is the frozen full 60-cell
recovery after r4/r5 stopped on intermittent provider connection errors;
science configuration is identical to r5, and the transport config adds
bounded non-fatal connection retries (max_attempts 3).
"""
...
def main() -> int:
    ...
    if protocol_id != "carr_s_e2_v8_formal_r6":
        raise ValueError("r6 lane runner requires the frozen r6 protocol")
