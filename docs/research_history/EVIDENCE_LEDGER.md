...
## 2026-09-05 E1 过程代码修复与短运行（PILOT）

- 用户要求保留 24/100 户设计和已有消融、跨模型，先修复旧实现并看小样本。修复已接回 disastersociety 实际源码；原始结果未覆盖。
- 修复成员名册、重复出发、当前意图与约定时间匹配、明确同行者、逐人协商确认、持续私有计划及执行反馈。所有修复源文件、备份和哈希见 `/Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/experiment_repair_20260905/promoted_files.json`。
- 原模型 packy-deepseek-v4-flash，4 户/12 人/7 决策成员，seed7201，12 步完整真实运行：VALID；逐步约束违规 0，n_ok=13，n_cache=3，n_failed=0，n_fallback=0；15 次新供应商请求。暂时性服务错误恢复及先前尝试均留档。
- 实际首次出发 2 人，全员到安全区 1 户，多人共同约定及共同执行均为 0。此小样本不能被写成多人协商效果验证。
- 修复相关 61 项、共享内核/机制/Carr 28 项检查通过，共 89 项；旧冻结测试使用既有历史记录检查函数，历史哈希未修改。离线检查仅为 ENGINEERING_VALIDATION。
- 已核查旧 E1 五个主实验 run：都有提前执行等行为影响，需要修复后补跑才能支持新的过程结果；画像、事件、种子及历史日志保留。新 24/100 户配置与五项任务已准备，尚未启动全量。
- 结果、图表、实际轨迹、代码差异及检查范围：`/Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/experiment_repair_20260905/完成情况与结果.md`。论文正文与结果表未用试跑数值覆盖。


### 2026-09-05 E1 首个完整 seed 运行恢复（仍 RUNNING）

24 户 seed7201 attempt2 完成17/25步后，在step18因完整JSON后多出一个闭合括号而中断。已增加仅针对单个多余闭合括号、保持JSON字段不变的格式规范化，并记录 format_normalizations；相邻多个对象、附加文本、截断对象等仍不能静默通过。27项相关检查通过。attempt3已启动，复用153条成功缓存；前17步147条决策及完整事件快照与attempt2逐步完全一致。旧尝试未覆盖，尚未放行余下四项。依据：/Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/main_experiments_20260905/amendments/json_brace_normalization.json 与 replay_prefix_check.json。


### 2026-09-05 E1 首个完整 seed 验收通过，余下四项已放行

24户、25步、seed7201 attempt3 已完整 VALID，247次逻辑决策（94新响应+153精确缓存），失败和fallback均0。独立日志检查13个实际出发组、全部44名成员位置与31名决策成员、76次消息收件人投递事件、图边界、时序和6个多人约定版本均通过。15/31决策成员首次实际出发；11/24家庭全员到安全区；3/7多人决策家庭形成约定、2/7共同执行。封路反馈后的备用路线执行有日志支持。源代码、输入和模型冻结哈希核对通过，图已渲染检查。仅为一个完整run，机制/跨seed结论仍待矩阵；缓存恢复的耗时不作为全新调用效率。余下24户seed8301/9401、100户seed101/202已按用户授权串行放行。结果：/Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/main_experiments_20260905/results/主实验结果.md；验收：first_seed_review.json。


### 2026-09-05 E1 第二个完整 seed 通过（2/5）

24户 seed8301 已完整25步 VALID，188次新真实模型决策，失败/fallback均0。13个执行出发组及29次消息收件人投递的额外边界/时序检查通过，所有多人约定有真实接受记录，冻结输入一致。实际出发14/31决策成员，全员到安全区11/24户，多人约定3/7户、共同执行1/7户。seed9401已串行启动，两个100户运行仍待前序完成。逐seed结果及图：/Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/main_experiments_20260905/results/主实验结果.md。

- 2026-09-05 22:15 CST E1 repair batch: all three 24-household seeds 7201/8301/9401 accepted for 25 steps. Seed 9401: 224 model decisions, no failed/fallback decisions, 11 executed parties; independent all-member/route gate and additional graph/time/freeze check passed. Final departures 12/31, all-safe households 10/24, agreement households 3/7, joint executions 1/7. Its joint departure occurred before closure; post-closure alternate-route departures are individual, not evidence of a second post-closure household joint replanning case. Aggregate regenerated for 3/5 accepted runs and plot visually checked. 100-household seed 101 running; seed 202 pending. Evidence: /Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/main_experiments_20260905/results/主实验结果.md and runs/24hh_seed9401/attempt1/analysis/additional_manual_review.json.


### 2026-09-06 E1 100-household schema recovery (3/5 accepted; still RUNNING)

Seed101 attempt1 stopped at step10 after the provider reduced account concurrency to 3. Concurrency was reduced from 12 to 3. Attempt2 completed 11 steps, then stopped at step12: a resident without a vehicle proposed walking, and the first schema correction still returned null vehicle_id, violating the existing vehicle-only evacuation contract. Both attempts and their raw responses are preserved.

The gateway still defaults to one schema correction. This E1 batch explicitly allows at most two. Only the additional correction receives the existing E1 conditional execution guidance. Walking, transport, new resources, and rule fallback were not added. Initial prompts, first correction prompts, and JSON Schema were checked identical. Model, population, events, seed, metrics, and budget are unchanged. Forty-two targeted offline tests passed. All three accepted 24-household runs replayed 25 steps with byte-identical events under the revised code; the 100-household prefix replayed 11 byte-identical steps and reused all 478 successful cache entries. These are software checks, not new behavior evidence.

Attempt3 has started from initial state with exact cache reuse. Completion still requires 25 steps and independent acceptance before the original seed202 job continues. The batch remains 3/5 accepted. Evidence: /Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/main_experiments_20260905/amendments/conditional_schema_recovery/ (amendment.json, compatibility_check.json, replay_checks.json, real_prefix_check.json, launch.json). Previous freezes and reviews were archived; first_seed_review.json records compatibility and replay checks for the amended freeze.


### 2026-09-06 E1 provider credit exhausted (3/5 accepted; WAITING_FOR_PROVIDER_CREDIT)

100-household seed101 attempt3 completed 24/25 steps, then the original Packy provider reported exhausted credit (last reported balance approximately USD -0.109176). Step25 expected 57 decisions; 14 succeeded and 43 remain without valid responses. No complete step25 world state was committed. The successful cache contains 774 responses. Seed202 has not started; all paid experiment processes exited. Earlier attempts, error messages, and raw responses remain preserved. The partial run is not accepted and is not part of aggregate results.

Added narrow Chinese exhausted-credit markers to the backend fatal-error classifier so this billing condition no longer receives futile transport retries; concurrency/rate-limit errors remain retryable. Twenty-one targeted offline tests passed. A cache-only replay reused all 774 responses and reproduced the original 24 complete steps byte-for-byte without any network call. Successful inference paths, prompts, behavioral rules, model, population, seed, and metrics remain unchanged. Freeze and first-seed compatibility review updated with preserved prior versions.

Evidence: /Users/linnuo/tmp/disastersociety/paper/ieee_journal/working/main_experiments_20260905/amendments/provider_credit_stop/ (amendment.json, replay_checks.json, blocked_state.json). The automation remains registered but must stay quiet and must not make paid probes or restarts until the user reports restoration of the original provider credit. Recovery is attempt4 from attempt3/cache.sqlite, followed by the original seed202 only after acceptance. The batch remains 3/5 accepted; no completion claim is made.
